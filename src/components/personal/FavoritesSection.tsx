    ...
    const fetchFavorites = async () => {
      ...
      try {
        ...
        const { data: outfitsData, error: outfitsError } = await supabase
          .from("outfits")
          .select("id, name, image_url, created_at, is_favorite, user_id")
          .eq("user_id", user.id)
          .eq("is_favorite", true)
        ...
        const { data: clothesData, error: clothesError } = await supabase
          .from("clothes")
          .select("id, name, image_url, brand, category, created_at, is_favorite, user_id")
          .eq("user_id", user.id)
          .eq("is_favorite", true)
          .eq("archived", false)
          .order("created_at", { ascending: false });
        ...
        const clothesItems: FavoriteItem[] = clothesData ? clothesData.map((item) => ({
          id: item.id,
          name: item.name,
          type: 'clothes',
          image_url: item.image_url || null,
